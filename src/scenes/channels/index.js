...
export default {
	...
	onUpdate({ x1, y1 }, shader) {
		const nEchoes = Math.floor(lerp(N_ECHOES_MIN, N_ECHOES_MAX, x1));
		const frameDelayPerEcho = Math.floor(lerp(FRAME_DELAY_PER_ECHO_MIN, FRAME_DELAY_PER_ECHO_MAX, y1));
